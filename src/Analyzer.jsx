...
// cards, full pills on every button and badge) and one grotesque sans throughout, no serif
// anywhere. Two accents do all the work: a light navy blue is the primary (buttons, links,
// active states, always with deep-navy text on top when used as a fill), and lime is
// reserved for positive/live indicators. Coral is reserved strictly for flags.

import { useEffect, useRef, useState } from "react";
import { displayName, reasonParts } from "./format.js";

// ── Palette ──────────────────────────────────────────────────────────────────
// The ground is neutral: a light bone page, a charcoal block, and greys between.
// Colour is accent only — chartreuse to act on, sky to start, terracotta to warn,
// emerald to confirm — so the page itself never reads as tinted.
const P = {
  bone: "#F3F2EE", raise: "#EAE8E2", card: "#FBFAF7",
  charcoal: "#232420", ink: "#1F211E",
  chart: "#BA9F38", olive: "#6B5D1F", emerald: "#1D3B28",
  terra: "#A8452F", sky: "#B1BACA", pale: "#E1CC96",
};

// ── Tokens for the charcoal bands: light type on dark ground. ────────────────
const D = {
  ink: P.bone, muted: "#B4B2AA", faint: "#8C8A83",
  mint: P.chart, brass: P.chart, brassSoft: P.chart,
  glassBorder: "rgba(243,242,238,0.16)",
  lift: "#2E302A", // charcoal raised one step, for panels sitting on the band
  block: P.charcoal,
...
// ── Tokens for the bone page: dark type on light ground. Chartreuse is only 2.2:1
// on bone, so accent TEXT uses a deep olive shade of it and chartreuse itself is
// kept for fills, which carry charcoal ink. `pine` and `teal` are legacy names
// from the old dark theme; the values are what matter.
const L = {
  bg: P.bone, card: P.card, line: "#DFDDD5", lineSoft: "#EDEBE5",
  ink: P.ink, muted: "#4F5249", faint: "#61645C",
  pine: P.ink, teal: P.olive, mint: P.emerald, brass: P.olive,
  flag: P.terra, flagBg: "rgba(168,69,47,0.09)", flagBorder: "rgba(168,69,47,0.28)",
  good: P.emerald,
};
// Accent fills. A chartreuse, sky or sand fill always carries charcoal ink on top —
// never bone, which is what keeps these legible.
const A = { lav: P.chart, lavInk: P.charcoal, lime: P.pale, limeInk: P.charcoal, raised: P.raise };
...
// Solid block, square corners, no lift. Separation comes from the fill and the rule.
const card = (o = {}) => ({
  ...
  return (
    ...
    <div style={{ maxWidth: 980, margin: "30px auto 0", textAlign: "left" }}>
      <div style={{ display: "flex", gap: 8, maxWidth: 560, margin: "0 auto" }}>
        <div style={{ flex: 1, minWidth: 0, position: "relative" }}>
          <input value={q} onChange={onInput} onKeyDown={onKey}
            ...
            style={{ width: "100%", boxSizing: "border-box", fontFamily: sans, fontSize: 15, color: D.ink, background: "rgba(243,242,238,0.10)",
      ...
      <div style={{ display: "flex", gap: 7, flexWrap: "wrap", alignItems: "center", maxWidth: 560, margin: "10px auto 0" }}>
        ...
        {examples.map((x) => (
          <button key={x} onClick={() => { setQ(x); run(x); }} style={{ background: "rgba(243,242,238,0.08)", border: `1px solid ${D.glassBorder}`, color: D.muted, borderRadius: 999, padding: "4px 11px", fontFamily: sans, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{x}</button>
        ))}
      </div>
...
function HeroResult({ result, onStart, snaptrade }) {
  const panel = glass({ marginTop: 14, padding: "18px 20px", background: "rgba(243,242,238,0.09)", borderRadius: 14 });
  ...
  if (result.type === "fund" && result.analyzable === false) {
    return (
      <div style={panel}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <div style={{ fontFamily: serif, fontSize: 20, color: D.ink }}>{result.symbol} · <span style={{ color: D.muted, fontFamily: sans, fontSize: 15 }}>{result.name}</span></div>
          <span style={{ fontFamily: sans, fontSize: 11, fontWeight: 600, color: D.muted, background: "rgba(243,242,238,0.10)", borderRadius: 999, padding: "3px 10px" }}>NOT ANALYZED</span>
        </div>
...
function FundBreakdown({ groups, theme }) {
  ...
  const c = dark
    ? { ink: D.ink, muted: D.muted, faint: D.faint, link: A.lav, panel: "rgba(243,242,238,0.07)", border: D.glassBorder, surface: "rgba(243,242,238,0.05)", flag: D.flag }
  ...
  return (
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 8, alignItems: "start" }}>
      {groups.map((g) => {
        ...
        // The header right above already names the company, so drop the reason's own
        // "Company — " prefix the same way the stock view does.
        const openReason = openItem
          ? Object.values(reasonParts(openItem.reason, displayName(openItem.name))).filter(Boolean).join(" ")
          : "";
        return (
          <div key={g.key} style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: 14, overflow: "hidden" }}>
            ...
            {isOpen && (
              <div id={`grp-${g.key}`} style={{ padding: "0 14px 13px", display: "grid", gap: 10 }}>
                ...
                {openItem && (
                  <div style={{ background: c.panel, border: `1px solid ${c.border}`, borderRadius: 12, padding: "10px 12px", display: "grid", gap: 6 }}>
                    <div style={{ fontFamily: sans, fontSize: 12.5, color: c.ink, lineHeight: 1.5 }}>
                      <b>{openItem.ticker}</b> · {displayName(openItem.name)}
                    </div>
                    <div style={{ fontFamily: sans, fontSize: 12.5, color: c.muted, lineHeight: 1.5 }}>{openReason}</div>
...
function FlagCard({ flag, company, dark = false }) {
  const surface = dark ? "rgba(243,242,238,0.05)" : L.card;
      ...
      {state === "done" ? (
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input id="waitlist-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" aria-label="Email for the waitlist"
            ...
            style={{ flex: "1 1 200px", fontFamily: sans, fontSize: 14, padding: "9px 12px", borderRadius: 999, border: "1px solid rgba(35,36,32,0.25)", background: "rgba(243,242,238,0.90)", color: A.lavInk, outline: "none" }} />
...
function HotNews({ wrap }) {
  ...
  const arrowBtn = (side) => ({
    ...
    background: L.card, color: L.pine, fontFamily: sans, fontSize: 18, cursor: "pointer", display: "grid", placeItems: "center", zIndex: 2,
  });
          ...
          <div ref={trackRef} style={{
            ...
            {items.map((it, i) => (
              <a key={i} href={it.link} target="_blank" rel="noopener noreferrer"
                 style={{
                   ...card({ padding: 0, overflow: "hidden", borderRadius: 14 }), display: "block", textDecoration: "none", color: "inherit",
...
const METHOD_CATALOGUE = [
  ...
  ["Social & human rights", [
    ...
    { name: "Predatory lending", status: "Live", def: "High-cost consumer lending with predatory terms: payday, title, pawn, and subprime.", counts: "Triple-digit-APR lending, title loans, and lenders with enforcement actions defining the practice.", not: "Ordinary consumer credit, prime installment lending, mainstream banks.", ex: "World Acceptance, EZCORP, Enova, Credit Acceptance." },
    ...
    { name: "Self-reported supply-chain violations", status: "Live", def: "Labor violations a company's own supplier audits found and disclosed.", counts: "A company's own published audit findings. This is a lower bar than a government determination, but it is the company's own admission, remediation included.", not: "Third-party allegations the company hasn't itself confirmed.", ex: "Apple (its own 2025 supply-chain report disclosed 10 Core Violations in 2024)." },
...
function Methodology({ onStart }) {
  ...
  const StatusBadge = ({ s }) => (
    <span style={{ fontFamily: sans, fontSize: 10.5, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase",
      ...
      border: `1px solid ${s === "Live" ? "rgba(29,59,40,0.40)" : "rgba(243,242,238,0.20)"}`, borderRadius: 0, padding: "2px 7px", whiteSpace: "nowrap" }}>{s}</span>
  );
    ...
    <div style={{ fontFamily: sans, background: L.bg, minHeight: "100dvh" }}>
      ...
      <section style={{ background: L.card, borderTop: `1px solid ${L.line}`, borderBottom: `1px solid ${L.line}` }}>
        <div style={{ ...wrap, padding: "clamp(48px,7vw,72px) 24px" }}>
          <SectionHead title="The three layers of detection" sub="A company is flagged if any layer flags it. When layers overlap, the most precise reason wins." />
                ...
                {flags.map((f) => (
                  <div key={f.name} style={card({ padding: "20px 22px" })}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
                      ...
                      {f.contested && <span style={{ fontFamily: sans, fontSize: 10.5, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", color: L.muted, background: "rgba(243,242,238,0.08)", border: `1px solid ${L.line}`, borderRadius: 0, padding: "2px 7px" }}>Contested</span>}
                    </div>
      ...
      <section style={{ ...wrap, textAlign: "center", padding: "clamp(48px,8vw,90px) 24px" }}>
        ...
        <button onClick={onStart} style={startBtn(999, "16px 32px", 16)}>Get started</button>
      </section>
...
function LandingHome({ onStart, snaptrade, meta }) {
  ...
  return (
    <div style={{ fontFamily: sans, background: L.bg }}>
      ...
      <Canvas>
        <nav style={{ position: "sticky", top: 0, zIndex: 20, borderBottom: `1px solid ${D.glassBorder}`, background: D.block, }}>
      ...
      <section style={{ background: P.raise }}>
    ...
    <Canvas>
      <div style={{ minHeight: "100dvh", display: "grid", placeItems: "center", padding: "40px 20px" }}>
        <div style={glass({ width: "100%", maxWidth: 420, padding: "34px 32px" })}>
          ...
          {err && <DarkErr>{err}</DarkErr>}
          {info && <div role="status" style={{ marginTop: 12, fontFamily: sans, fontSize: 13, color: D.ink, background: "rgba(243,242,238,0.08)", border: `1px solid ${D.glassBorder}`, padding: "10px 12px", borderRadius: 0, lineHeight: 1.5, overflowWrap: "anywhere" }}>{info}</div>}
...
function Dashboard({ user, onSignOut, onGoHome }) {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: L.bg, fontFamily: sans, color: L.ink }}>
      {/* light top bar */}
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: L.bg, borderBottom: `1px solid ${L.line}` }}>
...
function Results({ analysis }) {
  ...
  return (
    <LSection n="3" title="What we found" sub={null}>
      ...
      {/* honest method note */}
      <div style={card({ padding: "14px 16px", marginTop: 16, background: L.lineSoft })}>
  ...
  return (
    <label style={{ display: "block" }}>
      ...
      <input type={type} value={value} placeholder={placeholder}
        ...
        style={{ width: "100%", fontFamily: sans, fontSize: 14, color: D.ink, background: "rgba(243,242,238,0.08)",
  ...
  return (
    <div ref={boxRef} style={{ position: "relative", touchAction: "pan-y", cursor: onScrub ? "crosshair" : "default" }}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" role="img"
        ...
        <path d={`${line} L${W},${H} L0,${H} Z`} fill={color} fillOpacity="0.16" />
      ...
      {hasMark && (
        <>
          <div aria-hidden style={{
            ...
            width: 1, background: "rgba(243,242,238,0.34)", pointerEvents: "none",
          }} />
          <div aria-hidden style={{
            position: "absolute",
            left: `${(activeIdx / (data.length - 1)) * 100}%`,
...
function StatCard({ label, value, sub, delta, accent = false, action }) {
  ...
  const dim = accent ? "rgba(10,26,51,0.62)" : L.muted;
  return (
    <div style={card({
      padding: "16px 18px", background: accent ? A.lav : L.card, borderRadius: 14,
...
function ProgressRow({ icon, title, subtitle, leftValue, rightValue, pct, tone = A.lav }) {
  ...
  return (
    <div style={card({ padding: "14px 15px", marginBottom: 10, display: "grid", gap: 9 })}>
      ...
      {p !== null && (
        <div style={{ height: 4, borderRadius: 0, background: L.line, overflow: "hidden" }}>
          <div style={{ width: `${p * 100}%`, height: "100%", borderRadius: 0, background: tone }} />
        </div>
      )}
