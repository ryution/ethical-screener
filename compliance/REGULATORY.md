...
> Engineering's plain-English read of where PlainStreet sits, not legal advice. Have counsel
> confirm before relying on it. It replaces the earlier CIP and AML drafts, which belonged
> to an abandoned model where PlainStreet opened brokerage accounts and moved money — the
> analyzer does neither.

## The analyzer: why it's clear

PlainStreet, as it exists today, **reads holdings and explains them.** That single fact keeps it
...
compensation**, as a **business**. PlainStreet gives no advice:
...
of how PlainStreet is monetized later. (Keep it that way: never add "suggested" holdings, model
...
No money moves through PlainStreet at all. We don't hold, transfer, or disburse funds.
...
PlainStreet is a read-only analytics tool over accounts the user already owns. If a future
...
If PlainStreet later adds round-up giving, the money question returns — but there's a proven,
...
- **Agent-of-payee.** The structure that keeps this out of money-transmission licensing is a
  written agency agreement appointing PlainStreet as the sponsor's agent to collect donations —
...
**PlainStreet itself needn't be a nonprofit** for any of this — RoundUp.org is a for-profit
